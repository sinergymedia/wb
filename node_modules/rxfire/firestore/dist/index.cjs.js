...
function docData(ref, idField) {
    return doc(ref).pipe(operators.map(function (snap) { return snapToData(snap, idField); }));
}
 ...
 */
var filterEmpty = operators.filter(function (changes) { return changes.length > 0; });
/**
...
function collectionChanges(query, events) {
    if (events === void 0) { events = ALL_EVENTS; }
    return fromCollectionRef(query).pipe(operators.map(function (snapshot) { return snapshot.docChanges(); }), filterEvents(events), filterEmpty);
}
/**
...
function collection(query) {
    return fromCollectionRef(query).pipe(operators.map(function (changes) { return changes.docs; }));
}
/**
...
function sortedChanges(query, events) {
    return collectionChanges(query, events).pipe(operators.scan(function (current, changes) { return processDocumentChanges(current, changes, events); }, []));
}
/**
...
function auditTrail(query, events) {
    return collectionChanges(query, events).pipe(operators.scan(function (current, action) { return current.concat(action); }, []));
}
/**
