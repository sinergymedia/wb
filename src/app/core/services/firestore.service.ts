...
export class FirestoreService {
  ...
  colIds$<T>(ref: CollectionPredicate<T>, queryFn?): Observable<any[]> {
    return this.col(ref, queryFn)
      .snapshotChanges()
      .pipe(
        map((actions: DocumentChangeAction<T>[]) => {
  ...
  geopoint(lat: number, lng: number): firestore.GeoPoint {
  ...
  getDeals() {
    return this.afs.collection('deals', ref => ref
  ...
  dealAggregation() {
    ...
    const aggregateDoc = this.afs.doc(`deals`);

    this.afs.collection(`deals`, ref => ref.orderBy('createdAt', 'desc')).valueChanges().pipe(
      take(1),
      tap(arr => {
        const data = {
          total: arr.length,
