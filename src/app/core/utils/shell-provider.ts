...
export class ShellProvider<T> {
  ...
  constructor(shellModel: T, dataObservable: Observable<T>) {
    ...
    // Put both delay and data Observables into a combineLatest operator
    // which will emit a value when both of them emit their value
    // so the intentional delay caused by the delayObservable doesn't get added to the time the dataObservable takes to complete
    combineLatest(
